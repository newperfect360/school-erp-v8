...
export default function Attendance({ initialClass = "", initialDivision = "", onNavigate }) {
  ...
  const visibleStudents = students.filter(s => (!classFilter || s.className === classFilter) && (!divisionFilter || s.division === divisionFilter) && `${s.name} ${s.student_name_mr || ""} ${s.grNo} ${s.rollNo}`.toLowerCase().includes(query.toLowerCase()));
