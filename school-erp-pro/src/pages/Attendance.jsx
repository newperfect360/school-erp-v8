...
export default function Attendance({ initialClass = "", initialDivision = "", initialStatus = "", onNavigate }) {
  ...
  const [drafts,setDrafts,reloadDrafts]=useStoredState(draftKey,{});
  const roster = readStored("erp_pro_students", []).filter(isActiveStudent).filter(student=>student.academicYear===year);
  const teachers = readStored("erp_pro_teachers", []);
  const [savedAttendance, , reloadAttendance] = useStoredState("erp_pro_attendance", {});
  ...
  const submitted=readStored(submissionKey,[]);
  const finalized=student=>submitted.some(entry=>entry.date===date&&entry.groupKey===classKey(year,student.className,student.division));
  ...
  const markVisiblePresent = () => {
    ...
    if(setDrafts({...drafts,...Object.fromEntries(visibleStudents.map(student=>[draftId(year,date,student.id),{...drafts[draftId(year,date,student.id)],status:"Present"}]))}))notify("Present marks saved as drafts. Review and submit the complete class.");
  };
  ...
  return <div className="core-page attendance-workspace">
    ...
    <AttendanceCorrection year={year} standard={classFilter} division={divisionFilter} date={date} onSaved={()=>{reloadAttendance();reloadDrafts()}}/><AttendanceFinalization year={year} standard={classFilter} division={divisionFilter} date={date} onSaved={()=>{reloadAttendance();reloadDrafts()}}/>
    {communication.panel}
