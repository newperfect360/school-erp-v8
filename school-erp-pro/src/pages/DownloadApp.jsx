...
function useRelease() {
  ...
  useEffect(() => {
    ...
    fetch('/downloads/android/release.json', { signal: controller.signal })
      ...
      .then(data => {
        if (data.packageId !== 'com.gbsschool.app' || !/^[a-f0-9]{64}$/.test(data.sha256) || !/^Perfect-Education-v[0-9A-Za-z.-]+\.apk$/.test(data.file)) throw new Error('Invalid release');
        setRelease(data);
...
export function DownloadAppCard() {
  ...
  if(developmentEnabled)return <section className="app-download-card" aria-label="Android app download"><span className="app-release-label">Perfect Education DEMO / TEST BUILD</span><h2>Perfect Education Android App</h2><p>Version 1.0.0-perfectedu-demo · Android 7.0+. Shared demo records with this computer; production Firebase is unchanged.</p><p>Connect the phone by USB and enable USB debugging, then run <code>adb reverse tcp:5178 tcp:5178</code>. Keep the Web development server running.</p><a className="app-download-button" href="/__school_demo/apk" download="Perfect-Education-debug.apk">Download current demo APK</a></section>;
  return <section className="app-download-card" aria-label="Android app download">
    ...
    <h2>Perfect Education Android App</h2>
    ...
    <p>Development test APK. Production school sign-in is not verified.</p>
...
export default function DownloadApp() {
  ...
  return <main className="app-download-page">
    ...
    <section className="app-download-hero">
      <div><span className="app-release-label">LEARNING · GROWING · TOGETHER</span><h1>Your school.<br/><em>Now on Android.</em></h1><p>A first look at the Perfect Education mobile experience.</p><p className="app-test-notice"><strong>Test/debug APK — UI preview only.</strong> Requires the local development server. Production authentication and physical-device workflows remain unverified. Do not enter production credentials.</p>
