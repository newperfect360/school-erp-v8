...
import { readStored, writeStored, useStoredState, commitStoredBatch } from "../storage";
...
export default function Results({ settings = {}, onNavigate }) {
  const { t } = useLanguage();
  ...
  const actor=useContext(CommunicationSession),[editing,setEditing]=useState(null),[correction,setCorrection]=useState({}),[reason,setReason]=useState('');
  const revise=(archive=false)=>{
    if(!reason.trim())return notify('A marks correction/archive reason is required.');
    const latest=readStored('erp_pro_results',[]).find(row=>row.id===editing.id);
    if(JSON.stringify(latest)!==JSON.stringify(editing))return notify('Marks changed. Reopen the edit form.');
    const input={...editing,...correction};const error=validateMarks(input);if(error)return notify(error);const row=calculatedResult(input);
    if(archive&&!window.confirm('Archive these marks? Original values and revision history will be retained.'))return;
    const {revisions:oldRevisions,...before}=editing;const at=new Date().toISOString();
    const next={...row,archivedAt:archive?at:null,updatedAt:at,revisions:[...(oldRevisions||[]),{before,reason:reason.trim(),actor:actor?.uid,at,action:archive?'Archive':'Correction'}]};
    if(saveResults(results.map(r=>r.id===editing.id?next:r))){setEditing(null);setPreview('');notify('Marks saved with revision history.');}
  };
  ...
  const confirm = () => {
    ...
    try {
      ...
      for (const item of review.rows) { const saved = { ...item.existing, ...calculatedResult(item.row), id: item.existing?.id || crypto.randomUUID(), updatedAt: new Date().toISOString(), archivedAt:null }; if(item.existing){const {revisions:prior,...before}=item.existing;saved.revisions=[...(prior||[]),{before,reason:"Reviewed Excel import",actor:actor?.uid,at:saved.updatedAt,action:"Correction"}];} next = item.existing ? next.map(r => r.id === saved.id ? saved : r) : [...next, saved]; }
  ...
  const visible = results.filter(r => !r.archivedAt && (!form.studentId || r.studentId === form.studentId || (!r.studentId && selection?.grNo === r.grNo)) && (!query || Object.values(r).join(" ").toLowerCase().includes(query.toLowerCase())));
  const marksheet = () => {
  ...
  return <div className="core-page"><PageHeading eyebrow="ACADEMIC PROGRESS" title="Exams & results" description="Student-linked marks, reviewed Excel import and printable progress. No name-only matching." />
    ...
    {visible.length ? <div className="table-scroll"><table><thead><tr><th>Student / GR</th><th>{t("Exam")}</th><th>{t("Subject")}</th><th>Marks</th><th>Grade</th><th>Result</th><th>Actions</th></tr></thead><tbody>{visible.map(r => <tr key={r.id}><td>{r.studentName} · {r.grNo}</td><td>{r.exam} · {r.component || "Theory"}</td><td>{r.subject}</td><td>{r.obtainedMarks}/{r.maxMarks} ({r.percentage}%)</td><td>{r.grade}</td><td>{r.pass ? "Pass" : "Fail"}</td><td><button onClick={()=>{setEditing(r);setCorrection({maxMarks:String(r.maxMarks),obtainedMarks:String(r.obtainedMarks)});setReason("")}}>Edit / Archive marks</button>{r.revisions?.length>0&&<details><summary>Marks history</summary>{r.revisions.map((v,i)=><p key={i}>{v.at} ? {v.reason} ? Previous: {v.before.obtainedMarks}/{v.before.maxMarks}</p>)}</details>}</td></tr>)}</tbody></table></div> : <EmptyState icon="chart" title="A clear picture of progress" description="Add subject marks or import the class marks sheet to begin." />}
    {editing&&<section className="school-panel workflow-panel" role="dialog" aria-label="Correct marks"><h3>{editing.studentName} ? {editing.subject}</h3><label>Corrected maximum marks<input type="number" value={correction.maxMarks} onChange={e=>setCorrection({...correction,maxMarks:e.target.value})}/></label><label>Corrected obtained marks<input type="number" value={correction.obtainedMarks} onChange={e=>setCorrection({...correction,obtainedMarks:e.target.value})}/></label><label>Marks correction reason<input value={reason} onChange={e=>setReason(e.target.value)}/></label><button onClick={()=>revise(false)}>Save marks correction</button><button onClick={()=>revise(true)}>Archive marks</button><button onClick={()=>setEditing(null)}>Cancel marks correction</button></section>}
