...
export default function Classwork() {
  ...
  const save = () => {
    if (!form.className.trim() || !form.subject.trim() || !form.classwork.trim()) {
      alert("इयत्ता, विषय आणि वर्गपाठ भरा");
      return;
    }
  ...
  const sendWhatsApp = (c) => {
    ...
    if (!/^(?:\+?91)?\d{10}$/.test(c.mobile.trim())) { alert("वैध 10 अंकी मोबाईल नंबर भरा"); return; }
    window.open(`https://wa.me/91${c.mobile.slice(-10)}?text=${encodeURIComponent(msg)}`, "_blank");
  };

  return (
    <div className="page">
      <h2>📝 वर्गपाठ + WhatsApp</h2>
      <div className="form-grid">
        ...
        <textarea placeholder="वर्गपाठ" value={form.classwork} onChange={(e) => setForm({ ...form, classwork: e.target.value })}></textarea>
      </div>
