...
export const resolveStudentReference=resolveReference;
export default function StudentLookup({ students, onSelect }) {
  ...
  const find = input => { const student = resolveStudentReference(input, students,readStored('schoolSettings',{}).tenantId); if (!student) return notify("No unique active student matches this QR / GR reference."); onSelect(student); stop(); };
  const scan = async () => {
