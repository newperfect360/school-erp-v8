...
export function matchPhotos(students, files, field='photoNumber') {
 const allowed=new Set(['photoNumber','photoFileName','grNo','admissionNo','id']);if(!allowed.has(field))throw Error('Invalid photo matching field.');
 const index=new Map();for(const s of students.filter(s=>!s.archivedAt)){const value=key(field==='photoFileName'?String(s[field]||'').replace(/\.[^.]+$/,''):s[field]);if(value){const list=index.get(value)||[];list.push(s);index.set(value,list)}}
