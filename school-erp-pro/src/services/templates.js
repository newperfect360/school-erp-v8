...
export const placeholders = [...new Set(["student_name", "student_name_marathi", "gr_number", "admission_number", "roll_number", "standard", "division", "dob", "parent_name", "mobile", "institution_name", "school_name", "school_address", "academic_year", "issue_date", "certificate_number", "reason", "last_class", "progress", "conduct", "content", "photo", "school_logo", "qr", "subject_marks", "roster_table", "total", "percentage", "grade", "result", "remarks", ...Object.values(formatFields).flat()])];
export const escapeHtml = value => String(value ?? "").replace(/[&<>"']/g, char => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[char]);
export const baseFormatCss = "body{font-family:Arial,'Noto Sans Devanagari',sans-serif;color:#203a55;padding:32px;line-height:1.65}header{text-align:center;border-bottom:2px solid #203a55;padding-bottom:16px}h1{font-size:24px}h2{text-align:center}table{border-collapse:collapse;width:100%;margin:18px 0}td,th{border:1px solid #cbd5df;padding:8px;text-align:left}img{max-width:100px;max-height:110px;object-fit:contain}header .institution{white-space:pre-line;font-size:13px;margin:4px 0}header h1{font-size:22px;line-height:1.55;margin:8px 0}header img{width:72px;height:78px;object-fit:contain}footer{margin-top:48px;display:flex;justify-content:space-between}.draft{color:#925b22;font-size:12px;text-align:center;border:1px dashed #ba9e70;padding:8px}.document{break-after:page}.document:last-child{break-after:auto}@media print{body{padding:12mm}}";
export function defaultFormat(type) {
  ...
  return { type, name: type, approved: false, version: 1, css: baseFormatCss, html: `<header>{{school_logo}}<p class="institution">{{institution_name}}</p><h1>{{school_name}}</h1><p>{{school_address}}</p></header><h2>${type}</h2><p>No: {{certificate_number}} · Date: {{issue_date}}</p>${identity}${specialized[type] || details}<footer><div>{{qr}}<small>Internal record reference</small></div><p>Authorized school signature<br/>____________________</p></footer>` };
}
...
export function renderFormat(template, context) {
  ...
  const content = header + template.html.replace(/<header\b[^>]*>[\s\S]*?<\/header>/gi,'');
  const rendered = content.replace(/\{\{\s*([a-z_]+)\s*\}\}/g, (_, key) => ["photo", "school_logo", "qr", "subject_marks", "roster_table"].includes(key) ? context[key] || "" : escapeHtml(context[key]));
